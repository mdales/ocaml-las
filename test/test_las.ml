...
let test_wrong_magic () =
    ...
    Alcotest.(check ok_header) "wrong magic" (Error Las.Invalid_file_signature) result
...
let test_truncated_input () =
    ...
    Alcotest.(check ok_header) "truncated input" (Error (Las.Truncated "File Signature")) result
